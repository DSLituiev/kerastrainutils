...
class ModelCheckpoint(Callback):
    ...
    def __init__(self, filepath, monitor='val_loss', verbose=0,
        ...
        self.individual_epochs = individual_epochs
    ...
    def save_op(self, filepath, epoch=None):
    ...
    def on_epoch_end(self, epoch, logs=None):
        """
        change: 
            save_best_only OR period:
                if model improved -> save 
                OR
                if period passed  -> save
        """
        self.epoch = epoch
        logs = logs or {}
        ...
        if self.save_best_only:
            ...
            if self.median_kernel:
                self.metric_q[epoch % self.median_kernel] = current
                #self._qindex += 1
                #if self.metric_q.qsize() > self.median_kernel:
                #    self.metric_q.get()
                current_median = np.nanmedian(self.metric_q)
                if self.monitor_op(current_median, self.best):
                    if self.verbose > 0:
                    ...
                    saved = True
                    self.epochs_since_last_save = 0
                    self.save_op(filepath, epoch)
            if saved is not True and \
                ((not self.median_kernel) and (self.individual_epochs is None))  or \
                (self.individual_epochs is True):
        ...
        if self.period_ is not None and (self.epochs_since_last_save >= self.period_):
            self.epochs_since_last_save = 0
        #else:
            if self.verbose > 0:
                print('\nEpoch %05d: saving model to %s' % (epoch + 1, filepath))
